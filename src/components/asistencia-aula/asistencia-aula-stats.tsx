...
import { AsistenciaAulaRecord, FilterStatusAula } from "@/hooks/use-asistencia-aula";
...
interface StatCardProps {
    ...
    colorClass: string; 
    status: FilterStatusAula | 'incidentes';
    isActive: boolean;
    onClick: () => void;
}
...
interface AsistenciaAulaStatsProps {
    asistencia: Record<string, AsistenciaAulaRecord>;
    totalEstudiantes: number;
    incidentesCount: number;
    statusFilter: FilterStatusAula;
    onFilterChange: (status: FilterStatusAula) => void;
}

export function AsistenciaAulaStats({ asistencia, totalEstudiantes, incidentesCount, statusFilter, onFilterChange }: AsistenciaAulaStatsProps) {
    const stats = useMemo(() => ({
      ...
      const handleFilterChange = (status: FilterStatusAula) => {
        onFilterChange(status === statusFilter ? 'todos' : status);
      }

    return (
        <div className="grid gap-2 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6">
            <StatCard title="Estudiantes" value={stats.total} icon={Users} colorClass="bg-blue-500" status="todos" isActive={statusFilter === 'todos'} onClick={() => handleFilterChange('todos')} />
